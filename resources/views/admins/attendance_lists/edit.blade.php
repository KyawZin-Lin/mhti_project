                    ...
                    <div class="card mt-3">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <p class="mt-3"><b>Edit Attendance List</b></p>
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.attendance_lists.update', $attendance_list->id) }}" method="POST"
                                autocomplete="off">
                                @csrf
                                @method('PUT')
                                        ...
                                        <select name="student_id" id="student_id" class="myselect form-control">
                                            ...
                                            @foreach ($students as $student)
                                                <option value="{{ $student->id }}"
                                                    @if ($student->id == $attendance_list->student_id) selected @endif>{{ $student->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                        ...
                                        <select name="academic_year_id" id="academic_year_id" class="myselect form-control">
                                            ...
                                            @foreach ($academic_years as $academic_year)
                                                <option value="{{ $academic_year->id }}"
                                                    @if ($academic_year->id == $attendance_list->academic_year_id) selected @endif>
