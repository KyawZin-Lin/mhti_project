        ...
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-12 d-flex justify-content-between mb-4 title-bar">
                    <div>Class List</div>
                    <div>
                        <a href="{{ route('admin.classrooms.create') }}"><button type="button"
                                class="btn btn-md btn-success"><b><i class="fas fa-plus-square"></i>
                                    New Class</b></button></a>
                    </div>
                </div>
                <div class="col-sm-2">
                    <h5>Total - {{ $classroomsCount }}</h5>
                </div>
                <div class="col-sm-10">
                        ...
                        <table class="table table-striped table-hover">
                            ...
                            <tbody>
                                @foreach ($classrooms as $classroom)
                                    <tr>
                                        <td>{{ $classroom->batch }}</td>
                                        {{-- <td>{{ $classroom->student != null ? $classroom->student->name : '' }}
                                        </td>
                                        <td>{{ $classroom->student != null ? $classroom->student->phone : '' }}
                                        </td>
                                        <td>{{ $classroom->academicYear != null ? $classroom->academicYear->name : '' }}
                                        </td> --}}
                                        <td>
                                            <a href="{{ route('admin.classrooms.edit', $classroom->id) }}"><button
